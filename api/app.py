...
async def upload_dataframe(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Ошибка обработки DataFrame: {str(e)}")
...
async def get_columns(request: ColumnsRequest):
    ...
    if df is None or df.empty:
        ...
        raise HTTPException(
            status_code=404, detail="DataFrame пуст или не инициализирован")

    try:
        logger.info(
            f"Колонки датасета по которым берем срез: {str(request.columns)}")
        result_df = df[request.columns]
    except KeyError as e:
        logger.error(f"Столбец {str(e)} не найден")
        raise HTTPException(
            status_code=400, detail=f"Столбец {str(e)} не найден")
...
async def predict(model_id: str, request: Request):
    ...
    if model_id not in models:
        raise HTTPException(
            status_code=404, detail=f"Модель с ID '{model_id}' не найдена.")
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Ошибка обработки данных или выполнения инференса: {str(e)}")
