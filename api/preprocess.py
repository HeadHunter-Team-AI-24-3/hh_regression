...
cat_columns = [
    ...
    "working_time_intervals_0_name",
    "working_time_modes_0_name",
    "working_days_0_name",
    "branding_type",
    "branding_tariff",
    "department_name",
    ...
    "brand_snippet_background_gradient_color_list_1_position",
    "category",
]
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["working_time_intervals_0_name"] = df["working_time_intervals_0_name"].replace(
        "Можно сменами по\xa04-6\xa0часов в\xa0день", "Можно сменами по 4-6 часов в день"
    )
    df["working_time_intervals_0_name"].fillna("Unknown", inplace=True)

    df["working_time_modes_0_name"] = df["working_time_modes_0_name"].replace(
        "С\xa0началом дня после 16:00", "С началом дня после 16:00"
    )
    df["working_time_modes_0_name"].fillna("Unknown", inplace=True)

    df["working_days_0_name"] = df["working_days_0_name"].replace(
        "По\xa0субботам и\xa0воскресеньям", "По субботам и воскресеньям"
    )
    ...
    df["brand_snippet_background_gradient_color_list_0_position"] = df[
        "brand_snippet_background_gradient_color_list_0_position"
    ].notna()
    df["brand_snippet_background_gradient_color_list_1_position"] = df[
        "brand_snippet_background_gradient_color_list_1_position"
    ].notna()
...
def preprocess_data_for_model(df: pd.DataFrame, is_trained: bool) -> pd.DataFrame:
    ...
    df[["salary_gross", "employer_accredited_it_employer"]] = (
        df[["salary_gross", "employer_accredited_it_employer"]].astype(bool).astype(int)
    )
