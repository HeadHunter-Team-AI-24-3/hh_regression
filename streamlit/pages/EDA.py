...
st.set_page_config(page_title="Аналитика и EDA", page_icon="", layout="wide")  # Название в меню  # Иконка в меню
...
if not st.session_state.df.empty:
    ...
    if x_col and y_col:
        ...
        if chart_type == "Обычный":
            line_chart = alt.Chart(df).mark_line().encode(x=x_col, y=y_col, tooltip=[x_col, y_col])
        elif chart_type == "Шаговый":
            line_chart = (
                alt.Chart(df).mark_line(interpolate="step-after").encode(x=x_col, y=y_col, tooltip=[x_col, y_col])
            )
        else:  # Область
            line_chart = alt.Chart(df).mark_area().encode(x=x_col, y=y_col, tooltip=[x_col, y_col])
    ...
    if "salary" not in df.columns:
        st.error(
            "В датасете нет столбца 'salary'. Пожалуйста, загрузите корректный файл. Или воспользуйтесь построением своего графика из вашего датасета"
        )
    else:
        st.subheader("Постройте свой график, но с нашим датасетом")
        chart_type = st.selectbox("Выберите тип графика", options=["Scatterplot", "Boxplot", "Barplot"])
        columns = [col for col in df.columns if col != "Зарплата"]
        selected_column = st.selectbox("Выберите столбец для анализа", options=columns)
    ...
    if "salary" in df:
        ...
        # Зарплаты < 1 000 000
        sns.histplot(
            df.loc[df["salary"] < 1000000, "salary"],
            bins=30,
            kde=True,
            ax=ax[1],
    ...
    if "premium" in df:
        ...
        ax_premium.axhline(
            y=overall_mean_salary,
            color="red",
            linestyle="--",
            label="Средняя зарплата всего датасета",
        )
        ax_premium.legend()
    ...
    if "has_test" in df:
        ...
        average_salary_true = df[df["has_test"]]["salary"].mean()
        count_true = df[df["has_test"]].shape[0]

        average_salary_false = df[not df["has_test"]]["salary"].mean()
        count_false = df[not df["has_test"]].shape[0]
    ...
    if "address_metro_station_name" in df:
        ...
        summary = {"Количество": [], "Средняя зарплата": []}
        ...
        top_stations["Количество"].plot(
            kind="bar",
            color=plt_color,
            position=0,
            label="Количество",
            ax=ax_metro_stations3,
        )
        ...
        top_stations["Средняя зарплата"].plot(
            kind="bar",
            ax=ax2,
            color=plt_color_secondary,
            position=1,
        ...
        filtered_df_moscow = df[
            (df["area_name"] == "Москва")
            & (df["address_city"] == "Москва")
            & (df["address_metro_station_name"] != "Нет метро")
            & (df["address_metro_station_name"] != "Станция не указана")
        ]
        top_salary_stations_moscow = (
            filtered_df_moscow.groupby("address_metro_station_name")["salary"].mean().nlargest(5)
        )
    ...
    if "address_metro_line_name" in df:
        ...
        top_lines["Средняя зарплата"].plot(
            kind="bar",
            ax=ax2_lines,
            color=plt_color_secondary,
            position=1,
    ...
    if "employer_accredited_it_employer" in df:
        ...
        employer_accredited_it_employer_palette = {
            True: "green",
            False: "red",
            "Unknown": "black",
        }
        ...
        # Для наглядной визуализации возьмём salary < 99.9% выборки
        percentile_99 = df["salary"].quantile(0.999)
        ...
        means = [np.mean(groupA), np.mean(groupB), np.mean(groupC)]

        std_errors = [stats.sem(groupA), stats.sem(groupB), stats.sem(groupC)]
        ...
        ax_accredited_it3.set_title(
            "Средняя заработная плата по аккредитации работодателя с доверительными интервалами"
        )
    ...
    if "schedule_name" in df:
        ...
        ax_schedule_name2.set_title("Средняя зарплата по schedule_name с доверительными интервалами")
        ...
        st.write(
            "Полный день. Самый частый график, который предлагает работодатель - 61,7%. При этом занимает 3 место по ЗП: медианное значение состовляет 70.000 рублей, 50% данных лежит в пределах 50к - 100к, max значение по усам достигает ~180к."
        )
        st.write(
            "Сменный график. 17.6% от всех графиков. Последнее место по ЗП: медианное значение состовляет 62.010 рублей, 50% данных лежит в пределах 45.000 - 87.500, max значение по усам достигает ~140к."
        )
        st.write(
            "Удаленная работа. Самый редкий график рабочего дня, который предлагает работодатель - 5.1%. Занимает 4 место по ЗП: медианное значение состовляет 58.500 рублей, 50% данных лежит в пределах 42.250 - 95.000, max значение по усам достигает ~160к."
        )
        st.write(
            "Гибкий график. 7% от общего объёма вакансий. Занимает 2 место по ЗП: медианное значение состовляет 72.600 рублей, 50% данных лежит в пределах 43.400 - 120000, max значение по усам достигает ~240к."
        )
        st.write(
            "Вахтовый метод. 8.6%. Очевидно 1 место по ЗП: медианное значение состовляет 155.000 рублей, 50% данных лежит в пределах 120.000 - 197.500, max значение по усам достигает ~320к."
        )
    ...
    if "professional_roles_0_name" in df:
        ...
        mean_salaries = (
            df[df["professional_roles_0_name"].isin(top_roles[:20])]
            .groupby("professional_roles_0_name")["salary"]
            .mean()
        )
        ...
        for index, value in enumerate(role_counts.index):
            ...
            barplot.text(
                role_counts.values[index],
                index,
                f"Средняя ЗП: {mean_salary:.2f}",
                va="center",
                ha="left",
                fontsize=9,
            )
        ...
        barplot = sns.barplot(
            x=sorted_roles_by_salary.values,
            y=sorted_roles_by_salary.index,
            palette=color_palette,
            ax=ax_prof_name2,
        )
        ...
        for index, value in enumerate(sorted_roles_by_salary.index):
            ...
            barplot.text(
                sorted_roles_by_salary.values[index],
                index,
                f"{mean_salary:.2f}",
                va="center",
                ha="left",
        ...
        mean_salaries = (
            df[df["professional_roles_0_name"].isin(top_roles)].groupby("professional_roles_0_name")["salary"].mean()
        )
    ...
    if "experience_name" in df:
        ...
        sns.barplot(
            y=df["experience_name"].index,
            x=df["experience_name"],
            palette=color_palette,
            ax=ax_experience_name1,
        )
    ...
    if "employment_name" in df:
        ...
        sns.barplot(
            y=df["employment_name"].index,
            x=df["employment_name"],
            palette=color_palette,
            ax=ax_employment_name1,
        )
